...
## Start a new project in one sentence

For example:

> Use Jarvis to build a farm equipment booking platform for farmers.

Jarvis turns the sentence into a bounded delivery goal, inspects repository truth
and reusable components, identifies the core journeys and visual source, and then
chooses the smallest safe build path. When contracts are stable and early product
visibility matters, it builds the shell, routes, navigation, visible pages, and
states first, then connects real backend behavior by journey. High-risk boundaries
such as authorization, payments, inventory, and migrations receive an early real
probe instead.

During implementation, Jarvis reuses existing component-library behavior even
when local styling must change. Product UI is written as final copy rather than
showing development placeholders such as `No API yet`, `Mock data`, or test text.
It batches coherent implementation work and verifies at focused, affected,
journey, and release boundaries. Cross-session work is reconciled from code, Git,
state, and fresh evidence before continuing.

The condensed path is:

```text
One-sentence goal
  -> Minimum product framing
  -> Repository and component-library discovery
  -> Core journeys and visual source
  -> Shell, routes, and visible pages
  -> Real backend integration by journey
